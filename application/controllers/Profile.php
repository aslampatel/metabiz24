...
class Profile extends CI_Controller
{
	...
	function dashboard()
	{
        ...
        // $DebitsTotal = $this->Generalmodel->get_total('amount', 'wallet', ['user_id'=> $query[0]->id, 'type'=> 'Debit','action'=>0])[0]->amount;
        // $CreditsTotal = $this->Generalmodel->get_total('amount', 'wallet', ['user_id'=> $query[0]->id, 'type'=> 'Credit','action'=>0])[0]->amount;
...
// 	prx(array_unique($totalTeamsUser));
	...
	function merchant_deals_management()
	{
		...
		//====Admin Mail====
		$uid = $this->session->userdata('is_userlogged_id');
		$query = $this->Generalmodel->show_data_id('user', $uid, 'id', 'get', '');
		$data['dashboard'] = $query;
		//==================================
	...
	{
		$id = $this->session->userdata('is_userlogged_id');
...
// $url = "https://api.bluedart.com/servlet/RoutingServlet?handler=tnt&action=custawbquery&loginid=NBM94763&awb=awb&numbers=81288425831&format=xml&lickey=jlsimgootnhjrpljfbsqgvqshogurnur&verno=1.3f&scan=1";
	...
	public function wallet()
	{
		...
		// @$totbal = $this->Generalmodel->get_total_2('amount','wallet','id',$uid,'pay_status','1');
		// @$totded = $this->Generalmodel->get_total('amount','wallet','id',$uid);
		// @$wallet_bal = (@$totbal[0]->amount - @$totded[0]->amount);
		$DebitsTotal = $this->Generalmodel->get_total_2('amount', 'wallet', 'user_id', $query[0]->id, 'type', 'Debit');
		// echo $this->db->last_query(); exit;
		$CreditsTotal = $this->Generalmodel->get_total_2('amount', 'wallet', 'user_id', $query[0]->id, 'type', 'Credit');
		$wallet_bal = ($CreditsTotal[0]->amount - $DebitsTotal[0]->amount);
		$data['history'] = $this->Generalmodel->show_data_id('wallet', $query[0]->id, 'user_id', 'get', '');
		$data['bal'] = $wallet_bal;
		$data['wallets'] = $this->Generalmodel->get_wallet_data($uid);
	...
	function deposit()
	{
		 $uid = $this->session->userdata('is_userlogged_id'); 
		$query = $this->Generalmodel->show_data_id('user_register', $uid, 'id', 'get', '');
		$data['wallet']  = $this->Generalmodel->show_data_with_multiple_condition2('wallet', $query[0]->id, 'user_id','investment','transaction_type', 'get', '');
		// echo pairCalculation($query[0]->id); exit;
		$data['dashboard'] = $query;
		$data['title'] = "Deposit";
		$this->load->view('common/header', $data);
	...
	function process_deposit()
	{
		$uid = $this->session->userdata('is_userlogged_id');
		$query = $this->Generalmodel->show_data_id('user_register', $uid, 'id', 'get', '');
		$wallet = $this->Generalmodel->show_data_with_multiple_condition2('wallet', $query[0]->id, 'user_id','investment','transaction_type', 'get', '');
		if(empty($wallet)){
			$type = 'investment';
		}else{
			$type = 0;
		...
		if(empty($wallet)){
			$query = $this->Generalmodel->show_data_id('user_register', $uid, 'id', 'update', ['account_status'=>'Active','account_active_date' =>date('Y-m-d h:i:s') ]);
		  //  echo $this->db->last_query();
		}else{
		}
