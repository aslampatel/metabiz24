            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="<?= base_url(); ?>/supercontrol/Cost_breakup_management/update" onsubmit="return calTotal()" class="row" method="post">
                            <!-- <input type="hidden" name="preTotal" value="<?= $cbm[0]->ammount + $cbm[1]->ammount; ?>" > -->
                            <div class="col-md-12 row">
                                <?php //print_r($nncbm); exit; 
                                ?>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="exampleFormControlInput1">Registration Price</label>
                                        <input type="number" class="form-control" name="rPrice" id="rPrice" placeholder="Price" value="<?= $cbm[0]->ammount; ?>">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="exampleFormControlInput1"> Reward Point</label>
                                        <input type="number" class="form-control" name="rpPrice" id="rpPrice" placeholder="Price" value="<?= $cbm[1]->ammount; ?>">
                                    </div>
                                </div>
...
<script>
    function calTotal(){
        var rpPrice = parseInt($("#rpPrice").val());
        var rPrice = parseInt($("#rPrice").val());
        // alert((rpPrice + rPrice));
        if((rpPrice + rPrice) <= <?=$cm[0]->ammount; ?>){
            return true;
        }else{
            Swal.fire({
            title: 'Warning',
            text: 'Cost Breakup Amount is grater than <?=$cm[0]->ammount; ?>.',
            icon: 'error',
